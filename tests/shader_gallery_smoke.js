...
const rain = process.argv[4] ? path.resolve(process.argv[4]) : path.join(path.dirname(external), "Interactive_Rain.frag");
...
const context = {
    ...
    // Native I/O boundary: these imports have no generic A-D buffer files.
...
for (const mouseEnabled of [false, true]) {
    context.cfg_mouseEnabled = mouseEnabled;
    for (const file of [path.join(ui, "Shaders/Tokyo.frag"), path.join(ui, "Shaders/Dusti.frag"), external, rain, external]) {
        context._applyShaderFromPathImpl(pathToFileURL(file).href, path.basename(file));
        assert.equal(context.cfg_selectedShaderPath, pathToFileURL(file).href);
        assert.equal(context.cfg_mouseEnabled, mouseEnabled, "Gallery selection must retain mouse permission");
        ...
        assert.deepEqual(channels, file === external || file === rain ? [0, -1, -1, -1] : [-1, -1, -1, -1]);
        for (const buffer of ["A", "B", "C", "D"]) {
            assert.equal(context["cfg_useBuffer" + buffer], false, "Native rain must not enable generic buffers");
        }
    }
}
console.log("Gallery round-trip: Tokyo -> Dusti -> Heartfelt No Heart -> Interactive Rain -> Heartfelt No Heart passed (mouse off/on).");
